...
# # Platform configs with URLs (from your codes)
# PLATFORMS = {
#     "ycombinator": {
...
#         web_dev_jobs = []
#         max_jobs = 50  # New: Max limit to avoid overload, increase if needed
#         for i, card in enumerate(job_cards[:max_jobs]):  # Process up to 50 (remove [:max_jobs] for all)
#             try:
#                 text_content = (await card.inner_text()).strip()
#                 if not text_content:
#                     continue
#                 has_web_dev_keyword = any(keyword in text_content.lower() for keyword in WEB_DEV_KEYWORDS)
#                 if has_web_dev_keyword:  # Comment this if for all jobs without filter
...
#                     # Fetch job description (as requested)
...
#                         print(f"✅ Found web dev job on {platform_name}: {title} at {company}")
...
#         print(f"\n{'='*60}")
#         print(f"FOUND {len(flat_jobs)} WEB DEVELOPMENT JOBS:")
#         print(f"{'='*60}")
