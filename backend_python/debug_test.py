...
async def debug_single_job(context, job_url):
    ...
    try:
        ...
        try:
            ...
            for i, elem in enumerate(title_elements[:5]):  # Only first 5
                elem_text = await elem.text_content()
                elem_class = await elem.get_attribute('class')
                print(f"[DEBUG] Title element {i+1} class: {elem_class}")
                print(f"[DEBUG] Title element {i+1} text: {elem_text[:100] if elem_text else 'None'}")
        except Exception as e:
        ...
        try:
            ...
            for i, elem in enumerate(company_elements[:5]):  # Only first 5
                elem_text = await elem.text_content()
                elem_class = await elem.get_attribute('class')
                print(f"[DEBUG] Company element {i+1} class: {elem_class}")
                print(f"[DEBUG] Company element {i+1} text: {elem_text[:100] if elem_text else 'None'}")
        except Exception as e:
        ...
        try:
            ...
            for i, elem in enumerate(desc_elements[:3]):  # Only first 3
                elem_text = await elem.text_content()
                elem_class = await elem.get_attribute('class')
                print(f"[DEBUG] Description element {i+1} class: {elem_class}")
                print(f"[DEBUG] Description element {i+1} text: {elem_text[:200] if elem_text else 'None'}")
        except Exception as e:
